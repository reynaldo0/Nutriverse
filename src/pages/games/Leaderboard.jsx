...
const RulesLeaderboard = ({ schools }) => {
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-14 max-w-6xl w-full z-10">
        ...
        >
          ...
          <ol className="list-decimal pl-5 space-y-4 text-[#3A2E17] text-lg md:text-xl leading-relaxed">
            <li>
              Pilih produk tanaman yang tersedia untuk menanam{" "}
              <span className="text-green-600 font-semibold">(Maksimal 4)</span>
            </li>
            <li>
              Setelah dipilih, akan diberikan bibit tanaman. Bibit tersebut{" "}
              <span className="text-green-600 font-semibold">harus disirami</span>{" "}
              agar menjadi pohon dan menghasilkan produk tanaman.
            </li>
            <li>
              Ketika sudah menjadi produk, tekan{" "}
            ...
            <table className="w-full divide-y divide-primary-100 min-w-[400px]">
              ...
              <tbody className="divide-y divide-primary-100">
                {schools.map((school, i) => (
                  ...
                  >
                    <td className="px-6 py-4 text-secondary-200 font-bold">{i + 1}</td>
                    <td className="px-6 py-4 text-secondary-200">{school.name}</td>
                    <td className="px-6 py-4 text-secondary-200">{school.users_count}</td>
            ...
            {schools.map((school, i) => (
              ...
              >
                <div className="flex justify-between items-center mb-2">
                  <span className="font-bold text-secondary-200 text-lg">#{i + 1}</span>
                  <span className="font-semibold text-primary-100">{school.points} poin</span>
                </div>
