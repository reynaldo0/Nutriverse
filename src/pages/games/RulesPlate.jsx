...
export default function RulesPlate({ schools }) {
  ...
  return (
    <section className="min-h-[70vh] flex flex-col items-center justify-center w-full px-4 pt-10 pb-10 relative bg-[#F7FDEB]">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-10 max-w-5xl w-full z-10">
        ...
        >
          <h2 className="text-3xl md:text-4xl font-extrabold text-[#3A2E17] mb-4 text-center">
            Cara Main
          </h2>

          <ol className="list-decimal pl-5 space-y-3 text-[#3A2E17] text-base md:text-lg leading-relaxed">
            <li>
              Pengguna memilih produk tanaman{" "}
              <span className="text-green-600 font-semibold">
                (1 produk = 5 pertanyaan acak)
              </span>
            </li>
            <li>
              Setelah memilih, produk muncul pada{" "}
              <span className="text-green-600 font-semibold">piring</span> dan
              pertanyaan muncul di sebelahnya.
            </li>
            <li>
              Jawaban benar mendapat{" "}
              <span className="text-green-600 font-semibold">+5 poin</span>,
              jawaban salah{" "}
              <span className="text-green-600 font-semibold">-2 poin</span>.
            </li>
          </ol>
        </div>
        ...
        >
          ...
          <div className="hidden md:block overflow-x-auto">
            <table className="w-full divide-y divide-primary-100 min-w-[350px]">
              <thead className="bg-primary-100">
                <tr>
                  <th className="px-4 py-2 text-left text-xs font-bold text-secondary-200 uppercase tracking-wider">
                    #
                  </th>
                  <th className="px-4 py-2 text-left text-xs font-bold text-secondary-200 uppercase tracking-wider">
                    Nama Sekolah
              ...
              <tbody className="divide-y divide-primary-100">
                {schools.map((school, i) => (
                  ...
                  >
                    <td className="px-4 py-2 text-secondary-200 font-semibold">
                      {i + 1}
                    </td>
                    ...
                    <td className="px-4 py-2 text-secondary-200 truncate max-w-[120px]">
                      {school.team_code}
                    </td>
                  </tr>
                ))}
          ...
          <div className="md:hidden flex flex-col gap-3">
            {schools.map((school, i) => (
              <div
                key={school.id}
                className="bg-white/80 backdrop-blur-sm p-3 rounded-xl shadow-md border border-primary-100 hover:shadow-lg transition"
              >
                <div className="flex justify-between items-center mb-1">
                  <span className="font-bold text-secondary-200 text-base">
                    #{i + 1}
                  </span>
                ...
                <h3 className="text-base font-bold text-secondary-200 truncate">
                  {school.name}
                </h3>
