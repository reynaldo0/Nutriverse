...
export default function GameTree({ items = [] }) {
  ...
  const [points, setPoints] = useState(() => {
    return parseInt(localStorage.getItem("nutritree_points")) || 0;
  });
  ...
  useEffect(() => {
    localStorage.setItem("nutritree_points", points);
  }, [points]);

  const questions = useMemo(() => {
    if (!treeItem) return [];
    return treeItem.questions.map((q) => ({
      ...q,
          ...
          {treeItem ? (
            <div className="flex flex-col items-center z-10">
              ...
              <div className="mt-2 text-lg font-bold text-green-900 text-center">
                {treeItem.name} ({currentQuestionIndex + 1}/{questions.length})
              </div>
            </div>
          ) : (
