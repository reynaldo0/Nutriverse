...
const Navbar = () => {
    ...
    return (
        <nav className="fixed top-4 left-1/2 -translate-x-1/2 z-50 w-[90%] md:w-[80%]">
                    ...
                    {mainMenu.map((item) =>
                        !item.children ? (
                            ...
                            <div key={item.name} className="relative group">
                                <button className="flex items-center space-x-1 text-secondary-200 font-bold">
                                    <span>{item.name}</span>
                                    <svg
                                        className="h-4 w-4"
                                        fill="none"
                    ...
                    {user ? (
                        ...
                        authMenu.map((item) => (
                            <a
                                key={item.name}
                                href={item.path}
                                className={`px-5 py-2 rounded-full ${item.name === "Masuk"
                                        ? "bg-secondary-200 text-white font-bold"
                                        : "border-2 border-secondary-200 text-secondary-200 font-bold hover:bg-secondary-200 hover:text-white"
                                    }`}
                            >
                                {item.name}
                            </a>
            ...
            {/* Mobile Menu */}
            <div
                className={`md:hidden fixed top-20 right-4 w-72 bg-white/95 shadow-lg rounded-xl px-6 py-5 space-y-4 transition-all ${isOpen
            ...
            >
                ...
                {mainMenu.map((item) =>
                    !item.children ? (
                        <a
                            ...
                            className="w-full text-gray-900 font-semibold hover:text-secondary-200"
                        >
                            {item.name}
                        </a>
                    ) : (
                        <div key={item.name} className="w-full space-y-2">
                            ...
                            {item.children.map((child) => (
                                <a
                                    ...
                                    href={child.path}
                                    onClick={() => setIsOpen(false)}
                                    className="pl-4 text-gray-700 hover:text-secondary-200"
                                >
                                    {child.name}
                                </a>
                            ))}
                ...
                {!user ? (
                    <div className="flex flex-col w-full space-y-2 pt-2 border-t border-gray-200">
                        <a
                            href="/login"
                            className="w-full text-center px-5 py-2 rounded-full bg-secondary-200 text-white font-bold"
                        >
                            Masuk
                        </a>
                        <a
                            href="/register"
                            className="w-full text-center px-5 py-2 rounded-full border-2 border-secondary-200 text-secondary-200 font-bold"
                        >
                            Buat Akun
                        </a>
                    </div>
                    ...
                    <div className="flex flex-col w-full space-y-2 pt-2 border-t border-gray-200">
                        <a
                            href="/profile"
                            className="w-full text-center px-5 py-2 rounded-full border border-gray-300"
                        >
                            Edit Profil
                        </a>
                        <button
                            ...
                            className="w-full text-center px-5 py-2 rounded-full border-2 border-red-500 text-red-500"
                        >
                            Keluar
                        </button>
                    </div>
